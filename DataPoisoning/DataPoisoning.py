...
def poison_with_out_of_distribution_data(dataset, columns_to_poison, ood_percentage=0.1):
    num_poisoned = int(ood_percentage * len(dataset))
    ood_data = {column: np.random.uniform(dataset[column].min() * 1.5, dataset[column].max() * 1.5, num_poisoned) for
                column in columns_to_poison}
